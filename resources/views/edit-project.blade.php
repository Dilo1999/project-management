        ...
        <main class="flex-1 min-w-0 min-h-0 overflow-y-auto p-8 bg-gray-50">
            <div class="max-w-3xl mx-auto">
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-500 mt-2">Update the project details below</p>
                </div>
                    ...
                    <form action="{{ route('projects.update', $project) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="assigned_to" name="assigned_to" required
                                ...
                                @foreach($users as $member)
                                    <option value="{{ $member->name }}" {{ old('assigned_to', $project->assigned_to) === $member->name ? 'selected' : '' }}>{{ $member->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" id="start_date" name="start_date" value="{{ old('start_date', \Carbon\Carbon::parse($project->start_date)->format('Y-m-d')) }}" required
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none">
                            </div>
                            <div>
                                <label for="end_date" class="block text-sm font-medium text-gray-700 mb-2">End Date</label>
                                <input type="date" id="end_date" name="end_date" value="{{ old('end_date', \Carbon\Carbon::parse($project->end_date)->format('Y-m-d')) }}" required
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none">
                            </div>
                        </div>
                        ...
                        @php
                            ...
                            $selectedStatus = old('status', $project->status);
                        @endphp
                        <div>
